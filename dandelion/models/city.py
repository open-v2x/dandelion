...
class City(Base, DandelionBase):
    __tablename__ = "city"

    province_code = Column(String(64), ForeignKey("province.code"))
    code = Column(type_=String(64), unique=True, index=True, nullable=False)
    name = Column(type_=String(64), nullable=False)

    areas: List[Area] = relationship("Area", backref="city")
    ...
    def to_all_dict(self, need_intersection):
        return {
            **self.to_dict(),
            "children": [v.to_all_dict(need_intersection) for v in self.areas],
        }
