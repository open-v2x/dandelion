...
def algo_module_name(db, algo_version_in, update=False):
    ...
    if not algo_name_in_db:
        if (
            update
            and algo_version_in.enable == ALGO_CONFIG.get(f"{module}_{algo}").get("enable")
            and algo_version_in.in_use == ALGO_CONFIG.get(f"{module}_{algo}").get("inUse")
        ):
            return
        algo_name_in_db = crud.algo_name.create(
            ...
            obj_in=schemas.AlgoNameCreate(
                ...
                name=algo,
                enable=ALGO_CONFIG.get(f"{module}_{algo}").get("enable"),
                in_use=ALGO_CONFIG.get(f"{module}_{algo}").get("inUse"),
            ),
        )
